...
- `index.html`：公开合作主页；样式与移动导航在 `assets/site.css`、`assets/site.js`。
...
- 内容最大宽度 1160px。
- 首屏由个人介绍、合作入口和当前身份两列构成；按用户反馈收紧顶部留白，桌面 32px、手机 24px，不设置最小视口高度。
- Work 使用横向目录条目，不使用卡片容器。
- 墨绿色用于研究愿景与页尾联系区块。

维护主页时应优先减少层级，而不是增加装饰。合作方向采用目录式分隔线，工程实践采用两列项目，个人工具放在页尾次要位置。
